...
def generate_pass(m_id,team_name,e_data,l_data):
    ...
    t_data =  df.loc[df['team_name'] == team_name].set_index('id')

    t_pass = t_data.loc[t_data['type_name'] == 'Pass']
   
    
    passes = []
    
    for i, event in t_pass.iterrows():
...
def passing_network(m_id,team_name,e_data,l_data):
    ...
    player_df = df[df.team_name == team_name].groupby('player_name').agg({'minute': [min, max]}).reset_index()
...
# endnote /title
    ...
    axs['title'].text(0.5, 0.7, 'Passing Networks by Real Madrid', color='#c7d5cc',
